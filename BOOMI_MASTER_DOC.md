...
## Controlled-Pilot Hardening Release (v2.0.0) - STEP 2-11 DONE

Goal: harden the Core MVP into a controlled-pilot-ready release (no rewrite). Step 14 delivers the final A-I report + build.

- STEP 2 - Question Classifier Fix (engine.js): isQuestionStart() - word-boundary phrase starters + first-word starters (why/what/how/when/where/which/who/whose/whom; "whats"->"what's"). Replaces substring matching in analyzeQuestion (+50) and _looksLikeFreshTurn. Prevents mid-sentence false "new turn" splits.
- STEP 3 - Candidate Transcript Dedupe: handleCandidateText(text, isFinal) - undefined=legacy append, true=append-once (last-final dedupe), false=held interim. audio-pipeline.js forwards {text, isFinal}.
- STEP 4-6 - Multi-Provider Router (provider-router.js, NEW): ProviderRouter (injected fetch/timers), classifyError/extractStatus, ERROR_CLASS (TRANSIENT/AUTH/BAD_REQUEST/CONFIG/UNKNOWN), BREAKER_STATE (HEALTHY/DEGRADED/RATE_LIMITED/COOLDOWN/OFFLINE), DEFAULT_ROUTER_CFG (ttftMs 500, streamingTotalMs 30000, jsonTotalMs 15000, cooldownMs 60000, probeAfterMs 15000, temperature 0.35). Groq->Gemini failover. TTFT: wait <=500ms for first chunk; once streaming, continue under total-timeout budget. Breaker: auth/config->OFFLINE, 429->RATE_LIMITED, transient->COOLDOWN, else DEGRADED; all skip until probeAfter (half-open). Lazy key closures (empty key skipped). index.html: callWithFallback returns {text, provider, fallback, latencyMs, ttftMs, attempts} and emits PROVIDER_FALLBACK; legacy Groq/Gemini helpers removed.
- STEP 7 - Emergency Local Fallback (engine.js): EMERGENCY_RESPONSES + EMERGENCY_EXPERIENCE_VARIANT; all 3 API-error catches deliver an instant emergency answer and keep the engine alive for the next question. Emergency wording never mentions provider/API/timeout/error.
- STEP 8 - Scenario Bank Repair + Ranked Retrieval: knowledge/scenarios.json was two concatenated arrays (unparseable -> silent local-RAG death). scripts/repair-scenarios.js (bracket-stack split) -> single valid 311-entry array (313 original, 2 dupes removed, 0 malformed, no content lost). _searchLocalScenarios now scored: keyword coverage 1.0 + contiguous-phrase 0.15 + type alignment 0.1 (cap 1.0, x0.8 length penalty); SCORE_CFG STRONG 0.8 / AMBIGUITY 0.15 / WEAK_FLOOR 0.3 / MAX_HINTS 3; weak matches feed [CANDIDATE LOCAL CONTEXT] hints into buildAnswerPrompt.
- STEP 9 - Pilot Privacy Diagnostics (diagnostics.js): privacy debug|pilot (DIAGNOSTICS_PRIVACY in .env); pilot strips candidate transcript/content keys (explicit PRIVACY_STRIP_KEYS allow-list, metric keys preserved); SESSION_SUMMARY adds localScenarioHits/cloudAnswers/emergencyFallbacks/providerFallbacks + answer-latency P50/P90/P95 via new ANSWER_DELIVERED { source } events at all four delivery points. flush() now awaits in-flight async writes (fixed flaky engine->logger test).
- STEP 10 - Minimal+Functional UI Polish (index.html): confidence red -> neutral slate; question line 12->14px; auto-scroll re-engages on each new final answer; ~60ms render-only streaming paint throttle. Header "BOOMI COMPANION" + live dot already present.
- STEP 11 - Branding: scripts/generate-logo.js (pure Node + zlib) -> assets/logo.svg/logo.png/logo.ico; productName "Boomi Companion", build.win.icon assets/logo.ico; appId unchanged for installer upgrade continuity.
- STEP 12 - Docs sync (this file). STEP 13 - Tests (test/provider-router.test.js + extensions). STEP 14 - npm test + npm run build + final A-I report.
