...
## [2.0.3] - Interview-Script Audit Fixes (truncated questions, diff answers, grounded Salesforce)
- **`engine.js` — Truncated-question fix ("How do you handle?" → dropped real question):** new `TRANSITIVE_TRAILERS` verb set catches short *how-form* questions that end on a complement-demanding verb (`handle`, `process`, `connect`, `configure`, …) even with a trailing `?` — they now classify **incomplete** (previously the `?` bypassed the incomplete-trailer check, so a 75-score spurious answer burned a cloud call while the real continuation was dropped). The continuation-merge gate (`processTranscript`) now merges any non-fresh-turn utterance onto a pending incomplete buffer **regardless of pause length** (was: only within `continueWindowMs` 1500ms), and `_mergeTranscript` strips the trailing `?` so `"How do you handle?" + "Extremely large data volumes in Boomi?"` becomes one natural question that finalizes (~99) and answers once. A how-form gate (`^how` + ≤6 words) keeps "Why did you choose that approach?" and "How does that work?" correctly complete.
- **`engine.js` + `knowledge/scenarios.json` — Diff answer instead of two separate answers:** `classifyQuestionType` comparison regex now includes `differ(?:s|ed|ent)?`, so "And how does it actually differ from a Molecule?" types as **comparison** (was: conceptual → generic "what is a molecule" answer). Added the `differ` keyword to the `boomi_atom_vs_molecule` scenario so the diff question finally clears the `WEAK_FLOOR` and injects local diff grounding into the prompt (combined with the Atom Q/A already in follow-context).
- **`engine.js` — Grounded Salesforce / referential questions:** the engine now remembers the most recent **non-question statement** (`lastTopic`, e.g. "you need to connect to Salesforce") and injects it as `[EARLIER TOPIC]` into the answer prompt when the question is referential and carries no domain keyword ("Like, what is the best way to do that?"). Previously that follow-up went to the LLM with **zero context** (the topic statement was never a question, so it was never stored) — which is why the Salesforce answer was completely wrong.
- **`engine.js` — Vocabulary + routing-order fixes:** `best practice(?:s)?\b|\bbest way\b` and the **best-practice rule moved BEFORE the "what is/what are" conceptual rule** ("what are the absolute best practices?" was falling into conceptual). `_normalizePhonetics` adds `try slash catch` → `try catch` (STT drift now hits the local Try/Catch scenarios) and `AtomCloud` → `Atom Cloud`.
- **`engine.js` — Answer-text logging:** `TURN_COMPLETED` / `ANSWER_DELIVERED` diagnostics now include the actual `answer` text so session logs can be validated for content correctness (was: only the question was logged).
- Test Suite: **183 tests passing (123 engine + 27 diagnostic + 23 provider-router + 10 audio, 0 failed)** — new cases for `handle?` incomplete, `differ` comparison, `best practices` plural, topic injection, and the long-pause fragment merge.
- Guardrails honored: `provider-router.js` (keys/models/fallback order) untouched.
